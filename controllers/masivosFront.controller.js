...
module.exports.masivosClientes = async (req, res ) => {
    ...
    const { idcredito, agente , telefono ,tipo , comentarios,identificador } = req.body
 

    try {
        const sql = `EXEC SP_IngresaMasivo 
        @idcredito = ? ,
        @agente = ? ,
        @telefono = ? ,
        @tipo = ? ,
        ...
        const response = await db.query(sql, {
            replacements: [idcredito, agente , telefono ,tipo , comentarios,identificador],
            type: QueryTypes.RAW
        });
