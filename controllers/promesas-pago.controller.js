...
const LogPromesasBancoAzteca = require("../models/log-promesas-banco-azteca.model");
...
exports.mostrarPromesas = async (req, res) => {
    ...
    const { cliente_unico } = params;

    const { page = 1, limit = 15 } = req.query;

    try {

        const promesas_pago = await PromesasPago.findAndCountAll({
            where: {
...
module.exports.obtenerPromesa = async (req, res) => {
    ...
    try {
        ...
        const promesa_pago = await PromesasPago.findOne({
            where: {
...
exports.nuevaPromesaPago = async (req, res) => {
    ...
    try {
        ...
        const promesa_pago = new PromesasPago({
            id_interaccion, cliente_unico,
...
module.exports.actualizar_promesa = async (req, res) => {
    ...
    const { estatus, id_promesa, id_interaccion, cliente_unico, telefono_contacto ,fecha_gestion, gestor, fecha_pago, tipo_pago, tipo_contacto, monto, comentario = '', tipo = '', id_firma } = req.body
    ...
    try {
        if (estatus === 'rota') {
            ...
            try {
                ...
                const log_promesa =  new LogPromesasBancoAzteca({
                    id_promesa : id_promesa ,
                    id_cliente : promesa.cliente_unico ,
    ...
    try {
        ...
        const pago = new Pagos(data_pago);
